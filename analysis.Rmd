...
mydf.temp <- data %>% 
  ...
  select(c(5, 7:21)) %>% 
  mutate_all(replace_na, 0) %>% 
  gather(key="SWEBOKArea", value = "publications", 2:16) %>% 
  group_by(Year, SWEBOKArea) %>% 
  summarise(total=sum(publications)) 

# filling in the missing years
mydf.temp.complete <- mydf.temp %>% 
  complete(Year=seq(1973,2016))

# plot
mydf.temp.complete %>% 
  ggplot(aes(x=as.factor(Year), fill=SWEBOKArea, y = total)) + 
  geom_bar(stat="identity") + 
...
data %>%  ggplot(aes(x=as.factor(Year), fill=research_method)) + geom_bar() + scale_fill_discrete(name="Research methods", labels = c("Mixed", "Qualitative", "Quantitative", ""), na.value = "transparent") + xlab("Year") + ylab("Publications") +   theme(axis.text.x = element_text(angle = 45, hjust = 1, size = 5))
