...
data <- data %>% filter(is.na(Exclude)) %>% select(-Exclude)
```
...
data %>% 
  ...
  xlab("SWEBOK areas") + ylab("Concept") + guides(fill=guide_legend(title="")) + 
  theme_minimal() +
...
data %>% 
  ...
  mutate(freq=ifelse(value==1 & value2==1, n, 0)) %>% 
  ggplot(aes(fct_relevel(Method, c("Qualit. measures","Fieldwork", "Interview", "Task-based", "Open observation", "Quantit. measures", "Task performance", "Physiological meas.", "Subjective ratings", "Behavioral meas.")), fct_rev(Concept), fill=freq)) + 
  geom_tile() +
  geom_vline(xintercept = 5.5, size=0.5, linetype="longdash", color="darkgrey") +
...
df.years <- df.concepts %>% count(Year)
...
ggplot() + 
  ...
  scale_y_continuous(labels=function(x)x*100, name="Publication %", sec.axis = sec_axis(name="Total publications", ~. * max(df.years$n), breaks=scales::pretty_breaks(n = 10))) + 
  xlab("Year")  + 
